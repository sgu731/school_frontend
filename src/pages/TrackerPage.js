  ...
  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', marginTop: '1rem' }}>
    {shownSubjects.map(subj => (
      <div key={subj} style={{ display: 'flex', alignItems: 'center' }}>
        <div style={{ width: 12, height: 12, background: colorMap[subj], marginRight: 6, borderRadius: 2 }} />
...
const TrackerPage = () => {
  ...
  useEffect(() => { fetchStudyRecords(); }, []);
  useEffect(() => { if (startHour > endHour) setEndHour(startHour); }, [startHour]);
  ...
  const weekBarData = useMemo(() => barData.filter(d => {
  ...
  const weekSubjects = useMemo(() => {
    ...
    weekBarData.forEach(day => {
      Object.keys(day).forEach(k => { if (k !== 'date') set.add(k); });
    });
  ...
  const nextWeek = () => canNext && setWeekStart(new Date(weekStart.getTime() + 7 * 86400000));

  const weekSlots = useMemo(() => {
    return records.filter(r => {
      const t = new Date(r.created_at);
      return t >= weekStart && t < weekEnd && (!filterSubj || r.subject_name === filterSubj) && t.getHours() >= startHour && t.getHours() <= endHour;
    }).map(r => {
  ...
  const groupedSlots = useMemo(() => weekSlots.reduce((m, s) => {
    const k = `${s.col}_${s.rowStart}`;
    return { ...m, [k]: [...(m[k] || []), s] };
  }, {}), [weekSlots]);

  const days = ['日','一','二','三','四','五','六'];
  const headerDates = useMemo(() => Array.from({length:7},(_,i)=>{
    ...
    <div style={{ padding: '2rem', maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{ flex: 1, minWidth: '400px' }}>
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={weekBarData}>
              ...
              <YAxis />
              <Tooltip formatter={(v, name) => [`${Math.floor(v * 60)}分${Math.floor((v * 3600) % 60)}秒`, name]} />
              {weekSubjects.map(name => (
                <Bar key={name} dataKey={name} stackId="a" fill={subjectColorMap[name]} />
              ))}
            </BarChart>
          </ResponsiveContainer>
          <SubjectLegend colorMap={subjectColorMap} shownSubjects={weekSubjects} />
        </div>
            ...
            <PieChart>
              ...
              >
                {weekPieData.map((entry, index) => (
                  <Cell key={`cell-${index}`} fill={subjectColorMap[entry.name]} />
                ))}
              </Pie>
            </PieChart>
          </ResponsiveContainer>
      ...
      <div style={{marginTop:'1.5rem', display:'flex', flexDirection:'column', gap:'1rem', alignItems:'center'}}>
        <div style={{ display: 'flex', gap: '1rem' }}>
            <div>
            <label style={{ marginRight: 6 }}>開始時間：</label>
            ...
            <div>
            <label style={{ marginRight: 6 }}>結束時間：</label>
        ...
        <div>
            <label style={{ marginRight: 6 }}>顯示科目：</label>
            <select value={filterSubj} onChange={e => setFilterSubj(e.target.value)}>
            <option value=''>全部科目</option>
            {Object.keys(subjectColorMap).sort().map(subj => (
      ...
      <div style={{marginTop:'2rem'}}>
        <div style={{ display: 'grid', gridTemplateColumns: '60px repeat(7, 1fr)', textAlign: 'center', fontWeight: 500 }}>
          <div></div>
          {headerDates.map((t, i) => <div key={i}>{t}</div>)}
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '60px repeat(7, 1fr)', gridTemplateRows: `repeat(${(endHour - startHour + 1) * 2}, 20px)`, border: '1px solid #ddd' }}>
          {Array.from({ length: (endHour - startHour + 1) * 2 }).map((_, i) => {
            const hour = startHour + Math.floor(i / 2);
            return i % 2 === 0 ? (
              <div key={`time-${i}`} style={{ gridColumn: 1, gridRow: `${i + 1}`, fontSize: 10, borderRight: '1px solid #ddd', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{`${hour.toString().padStart(2, '0')}:00`}</div>
            ) : (
              <div key={`half-${i}`} style={{ gridColumn: 1, gridRow: `${i + 1}`, borderRight: '1px solid #ddd' }} />
            );
          })}
          ...
          {Object.entries(groupedSlots).map(([key, group]) => {
            ...
            return (
              <div key={key} style={{ gridColumn: col + 2, gridRow: `${rowStart - startHour * 2 + 1} / span ${span}`, display: 'flex' }}>
                {group.map((s, i) => (
                  <div
                    key={i}
                    title={`${s.subject} ${s.startTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} – ${s.endTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`}
                    style={{ flex: 1, background: subjectColorMap[s.subject], color: '#fff', fontSize: 10, padding: 2, borderRadius: 4, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                  >
                    {s.subject}
                  </div>
                ))}
